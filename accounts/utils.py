...
#create a more dynamic function instead of having 3 functions
...
def send_notification(mail_subject, mail_template, context):
    ...
    # Check if to_email is already a list for business order email confirmation
    if isinstance(to_email, list):
        recipient_list = to_email
    else:
        recipient_list = [to_email]

    mail = EmailMessage(mail_subject, message, from_email, recipient_list)
    mail.send()
